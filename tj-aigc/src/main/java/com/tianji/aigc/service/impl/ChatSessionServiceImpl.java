...
public class ChatSessionServiceImpl extends ServiceImpl<ChatSessionMapper, ChatSession> implements ChatSessionService {
    ...
    private static final int HISTORY_MESSAGE_NUM = 1000;
    ...
    public List<MessageVO> queryBySessionId(String sessionId) {
        ...
        // 从Redis中获取历史消息
        List<Message> messageList = this.chatMemory.get(conversationId, HISTORY_MESSAGE_NUM);
        // 过滤并转换消息列表
    ...
    public void deleteHistorySession(String sessionId) {
        // 删除数据库的数据
        LambdaQueryWrapper<ChatSession> chatSession = Wrappers.<ChatSession>lambdaQuery()
                .eq(ChatSession::getSessionId, sessionId)
                .eq(ChatSession::getUserId, UserContext.getUser());
        super.remove(chatSession);
